...
class AvatarHandler(BaseImageHandler):
    ...
    def process_avatar(self):
        ...
        show_processing_dialog(
            ...
            "Processing...",
            "Generating avatar...\nPlease wait...",
            lambda: self._execute_model_request(user_prompt),
        )
        # Exit
        self._exit_program()
    ...
    def _execute_model_request(self, user_prompt: str) -> bool:
        try:
            exemplar_images = [
            ...
            for returned_image in result.data:
                ...
                output_filename = f"avatar_{str(uuid4())}.png"
                with open(f"{avatar_dir_on_process}/{output_filename}", "wb") as f:
                    f.write(image_bytes)
                print(f"Saved avatar: {output_filename}")
            ...
            self.append_ad_hoc_comment_to_log(f"Error in avatar generation: {str(e)}")
